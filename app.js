...
function chooseComboButton(button) {
  const action = button.dataset.action ?? "";
  if (!action.startsWith("choose-")) return false;
  selectCombo(action.replace("choose-", ""), button.dataset.key, button.dataset.id);
  return true;
}
...
function bindEvents() {
  ...
  document.querySelectorAll("[data-combo-kind]").forEach((input) => {
    ...
    input.addEventListener("keydown", (event) => {
      ...
      const firstVisibleSuggestion = [...input.closest(".combo-wrap").querySelectorAll(".suggestion")].find((button) => !button.hidden);
      if (!firstVisibleSuggestion) return;
      event.preventDefault();
      chooseComboButton(firstVisibleSuggestion);
    });
    input.addEventListener("blur", () => window.setTimeout(() => input.closest(".combo-wrap")?.classList.remove("is-open"), 120));
  });
  document.querySelectorAll(".suggestion").forEach((button) => button.addEventListener("pointerdown", (event) => {
    event.preventDefault();
    chooseComboButton(button);
  }));
